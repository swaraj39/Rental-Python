...
def bookCar():
    ...
    print("Cost per Day ", cdf["Cost"].values[0])  # Print the value only
    total_cost = numberofdays * cdf["Cost"].values[0]  # Calculate total cost
    print("Total Rental Cost ", total_cost)
    print("^" * 40)
    k = input("Press C to continue")
    bdf = pd.read_csv("Cars Booked.csv")
    bfd = pd.read_csv("history.csv")
    n = bdf["Car Name"].count()
    m = bdf["Car Name"].count()
    bdf.loc[n] = [carname, mname, dateofbooking, numberofdays, total_cost, ""]
    bfd.loc[m] = [carname, mname, dateofbooking, numberofdays, total_cost, ""]
    bdf.to_csv("Cars Booked.csv", index=False)
    bfd.to_csv("history.csv", index=False)
...
def mainlogin():
    ...
    uname = input("Enter User Name:")
    pwd = input("Enter Password:")
    if uid == "S1" and uname == "Raj" and pwd == "392004":
           df = pd.read_csv("Users.csv")
           df = df.loc[df["User ID"] == uid]
...
def review():
    ...
    review = input("Please Review: ")

    df = pd.read_csv("Review.csv")
    sd = pd.read_csv("history.csv")
    sd = sd.loc[sd["Car Name"] == cname]
    if sd.empty:
          print("No Car Found")
          return False
    else:
          n = df["uname"].count()
          df.loc[n] = [uname, cname, review]
...
def showMenu():
    ...
    print("Press 16 - For Review")
    print("Press 17 - To Exit")
